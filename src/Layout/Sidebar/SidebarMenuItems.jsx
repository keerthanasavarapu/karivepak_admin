...
const SidebarMenuItems = ({menuItems, setMainMenu, sidebartoogle, setNavActive, activeClass }) => {
  const { layout } = useContext(CustomizerContext);
  const layout1 = localStorage.getItem("sidebar_layout") || layout;

  const id = window.location.pathname.split("/").unshift();
  const layoutId = id;
          ...
          {Item.Items.map((menuItem, i) => (
            <li className="sidebar-list" key={i}>
              ...
              {menuItem.type === "link" ? (
                <Link
                  to={menuItem.path}
                  className={`sidebar-link sidebar-title link-nav  ${CurrentPath.includes(menuItem.title.toLowerCase()) ? "active" : ""} ${menuItem.active ? "active" : ""}`}
                  onClick={() => toggletNavActive(menuItem)}
                  style={{ backgroundColor: menuItem.active ? "#aec6f4" : "" }}>
                  <SvgIcon className="stroke-icon" iconId={`stroke-${menuItem.icon}`} />
                  <SvgIcon className="fill-icon" iconId={`fill-${menuItem.icon}`} />
                  <span style={{ color: menuItem.active ? "#007F2D" : "" }}>{t(menuItem.title)}</span>
                  {menuItem.badge ? <label className={menuItem.badge}>{menuItem.badgetxt}</label> : ""}
                </Link>
                    ...
                    return (
                      <li key={index}>
                        {childrenItem.type === "sub" ? (
                          <a
                            href="javascript"
                            className={`${CurrentPath.includes(childrenItem?.title?.toLowerCase()) ? "active bg-red-500" : ""}`}
                        ...
                        {childrenItem.type === "link" ? (
                          <Link
                            ...
                            style={{ backgroundColor: childrenItem.active ? "transparent" : "", color: childrenItem.active ? "#d3178a" : "" }}

                            onClick={() => toggletNavActive(childrenItem)}>
                            {/* {t(childrenItem.title)} */}
                            <span style={{ color: childrenItem.active ? '#d3178a' : '' }}> {t(childrenItem.title)}</span>
