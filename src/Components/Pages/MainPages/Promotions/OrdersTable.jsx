...
const DataTableComponent = () => {
  ...
  const getPromotionslist = async () => {
    ...
    try {
      const token = await JSON.parse(localStorage.getItem("token"));
      await axios
        .get(`${baseURL}/api/promotion`)
        .then((res) => {
          console.log(res,"response in promotions")
          if (res?.data?.success && res) {
            console.log(res.data,"inside condition")
            setOrderData(res?.data?.data);
            setIsLoading(false);
          }
        });
  ...
  const orderColumns = [
    {
      name: "#PROMO_ID",
      selector: (row) => row["_id"],  // Use _id to select the field
      sortable: true,
      width: "150px",
      center: true,
      cell: (row) => (
        ...
        >
          {/* Displaying last 6 characters of the _id with "ID:" prefix */}
          ID:{row?._id?.slice(-6)}
        </div>
      )
    },
    {
      ...
      selector: (row) => row["title"],  // Use title to select the field
      sortable: true,
      ...
      cell: (row) => (
        <div className='ellipses_text_3'
          onClick={() => handleNavigate(row?._id)}
        >
          {/* Displaying the promotion title */}
          {row?.title}
        </div>
      )
    },
    {
      name: "Promo Code",
      selector: (row) => row["promoCode"],  // Use promoCode to select the field
      sortable: true,
      ...
      cell: (row) => (
        ...
        >
          ...
          {row?.promoCode || "N/A"}  {/* Show N/A if promoCode is not available */}
        </div>
      )
    },
    {
      name: 'Applicable Roles',
      selector: row => row?.applicableToRoles?.join(', '),  // Join array of strings into a comma-separated string
      width: "270px",
      headerStyle: (selector, id) => {
        return { textAlign: "left" };
      },
      cell: (row) => {
        const { applicableToRoles } = row;
        const MAX_DISPLAY_ROLES = 2;
        const rolesArray = applicableToRoles || []; // Ensure it's an array, even if empty
    
        if (rolesArray.length > MAX_DISPLAY_ROLES) {
          const pendingItemsCount = rolesArray.length - MAX_DISPLAY_ROLES;
          return (
            <>
              {rolesArray.slice(0, MAX_DISPLAY_ROLES).map((role, index) => (
    ...
    // Discount Amount Field
{
  ...
  selector: (row) => row["discountAmount"],  // Use discountAmount to select the field
  sortable: true,
  width: "150px",
  center: true,
  cell: (row) => (
    ...
    >
      ...
      {row?.discountAmount !== undefined ? `$${row?.discountAmount.toFixed(2)}` : "N/A"}
    </div>
  )
},
...
// Discount Percentage Field
{
  ...
  selector: (row) => row["discountPercentage"],  // Use discountPercentage to select the field
  sortable: true,
  width: "150px",
  center: true,
  cell: (row) => (
    ...
    >
      ...
      {row?.discountPercentage !== undefined ? `${row?.discountPercentage}%` : "N/A"}
    </div>
  )
},
{
  ...
  cell: (row) => (
    <div>
      {row?.maxDiscountAmount ? `$${row?.maxDiscountAmount.toFixed(2)}` : 'N/A'}  {/* Display max discount amount */}
    </div>
  ),
},

{
  ...
  selector: (row) => row["minPurchaseAmount"],
  sortable: true,
  center: true,
  cell: (row) => (
    <div>
      {row?.minPurchaseAmount ? `$${row?.minPurchaseAmount.toFixed(2)}` : 'N/A'}  {/* Display min purchase amount */}
    </div>
  ),
},


// Description Field (placed last)
{
  name: "Description",
  selector: (row) => row["description"],  // Use description to select the field
  sortable: true,
  width: "250px",  // Adjust width if needed
  center: true,
  cell: (row) => (
...
{
  name: "Status",
  selector: (row) => row["status"],  // Use status to select the field
  sortable: true,
  center: true,
  cell: (row) => {
    const customColors = {
      active: "success",  // Green for active
      inactive: "secondary",  // Grey for inactive
      expired: "danger",  // Red for expired
    };

    const statusKey = row?.status?.toLowerCase();
    const color = customColors[statusKey] || 'primary';  // Default to primary if no match
    return (
      <span style={{ fontSize: '13px' }} className={`badge badge-light-${color}`}>
        {capitalizeFirstLetter(row?.status)}  {/* Capitalize the first letter */}
      </span>
    );
  }
},
{
  name: 'Actions',
  cell: (row) => (
    <div className=' justify-content-end align-items-center' style={{ marginRight: '20px' }}>
      {/* Edit Button - Blue color */}
      <button
        className="bg-blue-500 text-white px-3 py-1 rounded-md text-sm mr-2 mb-2"
        onClick={() => handleNavigate(row?._id)}  // Replace with your edit logic
      >
        Edit
      </button>
      ...
      <button
        ...
        // onClick={() => handleDelete(row?._id)}  // Replace with your delete logic
      >
        Delete
      </button>
    </div>
  ...
  console.log(orderData,"outside all functions")
  return (
    <Fragment>
        ...
        <Row>
          <Col md={12} lg={12} xl={12} xxl={12}>
            <div className="file-content align-items-center justify-content-between">
              <H5 attrH5={{ className: "mb-0" }}>Promotions</H5>
              <div className="px-4">
