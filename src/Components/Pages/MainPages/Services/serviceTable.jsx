...
const ItemsTable = () => {
    ...
    const fetchItems = async () => {
            ...
            try {
                ...
                const products = await axios.get(`${baseURL}/api/services/`
                    // , {
                    // params: params,
                    // headers: {
                    //     Authorization: `${token}`,
    ...
    const orderColumns = [
                ...
                <>
                    ...
                    <Media className='d-flex'><Image attrImage={{ className: 'img-30 me-3', src: `${row?.variants?.length > 0 ? imageURL + row?.variants[0].variantImage : dummyImg}`, alt: 'Generic placeholder image' }} />
                        <Media body className="align-self-center">
                            <div className='ellipses_text_1'>{row?.serviceName}</div>
                        </Media>
                    </Media>
                </>
            ),
        ...
        {
            name: 'Service Status',
            selector: row => row.isActive ? 'Active' : 'Inactive', // Access and evaluate the isActive field
            sortable: true,
            center: true,
            width: "120px",
            cell: (row) => (
                <span
                    style={{ fontSize: '13px' }}
                    className={`badge ${row.isActive ? 'badge-light-success' : 'badge-light-danger'}`} // Apply different styles based on isActive value
                >
                    {row.isActive ? 'Active' : 'Inactive'} {/* Display status text */}
                </span>
            ),
        },
